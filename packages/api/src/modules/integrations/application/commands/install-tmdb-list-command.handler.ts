...
import { InstallTmdbListCommand } from './install-tmdb-list.command';
...
@CommandHandler(InstallTmdbListCommand)
export class InstallTmdbListCommandHandler
  implements ICommandHandler<InstallTmdbListCommand, InstallListResponseDto>
{
  constructor(
  ...
  async execute(command: InstallTmdbListCommand): Promise<InstallListResponseDto> {
    ...
    if (body.kind === 'builtin') {
      const def = TMDB_BUILTIN_LISTS.find((d) => d.listType === body.listType && d.type === body.type);
      if (!def) throw new BadRequestException('Invalid built-in TMDB list');

      config = await this.queryBus.execute(
        new FindExistingTmdbListQuery(userId, { tmdbListType: body.listType, type: body.type }),
      );
      ...
      if (!config) {
        config = await this.commandBus.execute(
          new CreateAddonConfigCommand(userId, {
            ...
            source: 'tmdb-list', tmdbListType: body.listType,
          }, maxAllowed),
        );
      }
    } else {
      ...
      config = await this.queryBus.execute(
        new FindExistingTmdbListQuery(userId, { tmdbListId: body.listId }),
      );
      ...
      if (!config) {
        config = await this.commandBus.execute(
          new CreateAddonConfigCommand(userId, {
            ...
            source: 'tmdb-list', tmdbListId: body.listId,
          }, maxAllowed),
        );
      }
    }
